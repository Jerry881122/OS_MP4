...
class FileSystem {
  ...
  ////////////////////// added code ////////////////////////////////
	OpenFileId OpenFile_MP1_(char *name)
	{
    OpenFile* fd;
    fd = Open(name);
    if(fd == NULL) return -1;
  ...
  int WriteFile_MP1_(char *buffer, int size, OpenFileId id){ 
  ...
  int ReadFile_MP1_(char *buffer, int size, OpenFileId id){
      if (id >= 20 || id < 0 || fileDescriptorTable[id] == NULL)
      {
          return -1;
      }
  ...
  int CloseFile_MP1_(OpenFileId id){
      if (id >= 20 || id < 0 || fileDescriptorTable[id] == NULL)
      {
          return 0;
      }
